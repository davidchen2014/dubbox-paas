		...
		if (applications != null && applications.size() > 0) {
			for (String application : applications) {
				...
				if (providers != null && providers.size() > 0 || consumers != null && consumers.size() > 0) {
					URL provider = (providers != null && providers.size() > 0 ? providers.iterator().next() : consumers
							.iterator().next());
				...
				row.add(providersSize == 0 ? "<font color=\"blue\">No provider</font>"
						: "<a href=\"providers.html?application=" + application + "\">Providers(" + providersSize
								+ ")</a>");

				int consumersSize = consumers == null ? 0 : consumers.size();
				consumersCount += consumersSize;
				row.add(consumersSize == 0 ? "<font color=\"blue\">No consumer</font>"
						: "<a href=\"consumers.html?application=" + application + "\">Consumers(" + consumersSize
								+ ")</a>");
				// 添加了一栏对外提供服务
				int total = 0;
				for (URL providerUrl : providers) {
					String _methods = providerUrl.getParameter(RestConstants.REST_METHOD_KEY);
					if (_methods != null && _methods.length() > 0) {
						total += _methods.split(",").length;
					}
				}
				row.add(providersSize == 0 ? "<font color=\"blue\">无</font>"
						: "<a href=\"restservices.html?application=" + application + "\">数量(" + total + ")</a>"
								+ "<a onclick=\"openDialogWin('pages/restservices.jsp?application=" + application + "');\" href=\"#\">发布(" + total + ")</a>");
					// <a onclick="alert('aaa'); openDialogWin(" restservices.jsp?application="sample-demo1&quot;);&quot;" href="#">发布(2)</a>			
				...
				row.add(efferentSize == 0 ? "<font color=\"blue\">No dependency</font>"
						: "<a href=\"dependencies.html?application=" + application + "\">Depends On(" + efferentSize
								+ ")</a>");
				...
				row.add(afferentSize == 0 ? "<font color=\"blue\">No used</font>"
						: "<a href=\"dependencies.html?application=" + application + "&reverse=true\">Used By("
								+ afferentSize + ")</a>");
				rows.add(row);
			}
		}
		return new Page("Applications", "Applications (" + rows.size() + ")", new String[] { "Application Name:",
				"Owner", "Providers(" + providersCount + ")", "Consumers(" + consumersCount + ")", "公开提供服务",
				"Depends On(" + efferentCount + ")", "Used By(" + afferentCount + ")" }, rows);
	}
